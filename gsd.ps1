...
Write-Output "Sources : nyrepo test"
...
            Write-Output "OSD Marker file removed"
        } else {
...
    Write-Output "Adobe Creative Cloud is not installed. Installing..."
    # Define download URLs for each architecture
    $urlX64   = "https://raw.githubusercontent.com/NYUAD-IT/nyrepo/refs/heads/main/rum_x64.zip"
    $urlARM64 = "https://raw.githubusercontent.com/NYUAD-IT/nyrepo/refs/heads/main/rum_a64.zip"

    # Detect System Architecture
    $arch = $env:PROCESSOR_ARCHITECTURE
    if ($env:PROCESSOR_ARCHITEW6432) {
        # Handles cases where a 32-bit PowerShell process runs on a 64-bit OS
...
        "*ARM64*" {
            Write-Host "Detected Architecture: ARM64" -ForegroundColor Yellow
            $downloadUrl = $urlARM64
        }
        "*AMD64*" {
            Write-Host "Detected Architecture: x64 (64-bit)" -ForegroundColor Yellow
            $downloadUrl = $urlX64
        }
        "*64*" {
            Write-Host "Detected Architecture: x64 (64-bit)" -ForegroundColor Yellow
...
        }
        Default {
            Write-Error "Unsupported or undetected architecture: $arch"
            exit 1
        }
    }

    # Define Temp Paths
    $zipPath = Join-Path -Path $env:TEMP -ChildPath "downloaded_archive.zip"
    $extractPath = Join-Path -Path $env:TEMP -ChildPath "extracted_app"

    try {
        # 1. Download using curl.exe for speed (follows redirects via -L)
        Write-Host "Downloading archive via curl..." -ForegroundColor Cyan
        curl.exe -sSL "$downloadUrl" -o "$zipPath"

        if (-not (Test-Path $zipPath) -or (Get-Item $zipPath).Length -eq 0) {
            Write-Error "Failed to download file or downloaded file is empty."
            exit 1
        }

        # 2. Extract ZIP archive to %TEMP%
        Write-Host "Extracting archive to $extractPath..." -ForegroundColor Cyan
        Expand-Archive -Path $zipPath -DestinationPath $extractPath -Force

        # 3. Find and silently install any MSI files in extracted folder
        $msiFiles = Get-ChildItem -Path $extractPath -Filter "*.msi" -Recurse

        if ($msiFiles.Count -eq 0) {
            Write-Error "No MSI file found in the extracted ZIP archive."
            exit 1
        }
...
                Write-Host "Installation of $($msi.Name) completed successfully." -ForegroundColor Green
            } else {
...
if ($hostname -match '^(ADUAE|NYUAD|ADAV)') { 
...
    Write-Output "Not NYUAD Managed PC. Skipping KMS activation."
