...
app_list=(
    "/Applications/Firefox.app,firefox,0"
    "/Applications/Adobe Acrobat Reader.app,adobe-acrobat-reader,0"
    "/Applications/Adobe Acrobat DC,adobe-acrobat-pro,0"
    "/Applications/Wireshark.app,wireshark,0"
    "/Applications/VMware Fusion.app,vmware-fusion,0"
    "/Applications/Thunderbird.app,thunderbird,0"
    "/Applications/VLC.app,vlc,1"
    "/Applications/Google Drive.app,google-drive,2"
    "/Applications/Cisco Jabber.app,cisco-jabber,1"
    "/Applications/zoom.us.app,zoom-for-it-admins,1"
    "/Applications/Google Chrome.app,google-chrome,1"
    "/Applications/Microsoft*.app,microsoft-office-businesspro,2"
    "/Applications/zoom.us.app,zoom,1"
    "/Library/Java/JavaVirtualMachines/zulu*,zulu,0"
)
...
for entry in "${app_list[@]}"; do
    ...
    # Microsoft Office specific verification
    if [[ "$cask_name" == "microsoft-office-businesspro" ]]; then
        office_apps=(
        ...
        missing_count=0
        for path in "${office_apps[@]}"; do
            [ ! -d "$path" ] && ((missing_count++))
        done
        if [ "$missing_count" -gt 0 ]; then
            echo "Detected $missing_count missing Microsoft Office apps. Forcing reinstall of $cask_name..."
            sudo -A -E -H -u "#$uid" env HOME="/Users/itops" bash -c "$brew_path uninstall --cask --force '$cask_name'" 2>/dev/null
        fi
    fi
